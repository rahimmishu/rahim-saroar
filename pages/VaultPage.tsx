  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        else if (view === 'gallery') setView('search');
        else navigate('/');
      }
    };
    ...
    return (
      <div className="relative flex flex-col items-center justify-center min-h-screen bg-black overflow-hidden">
        ...
        <div className="w-full max-w-xl mx-4 relative overflow-hidden
          bg-[#0a0a0a]/80 border border-white/10 shadow-2xl rounded-2xl backdrop-blur-2xl ring-1 ring-white/5">
          ...
          <div className="flex items-center px-6 py-5 border-b border-white/5">
            ...
            <kbd className="px-2 py-1 text-[10px] font-bold tracking-wider rounded bg-white/5 text-neutral-400 border border-white/5">
              ESC
            </kbd>
          </div>
          ...
          <div className="p-8 text-center min-h-[140px] flex flex-col items-center justify-center">
            {message ? (
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <p className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-purple-200">
                  {message}
                </p>
              </div>
            ) : (
          ...
          {galleryItems.map((item, idx) => {
            ...
            return (
              ...
              >
                    ...
                    <div className="flex items-center justify-center w-full h-full transition-colors bg-neutral-900 group-hover:bg-neutral-800">
                      {item.type === 'video' ? (
                        ...
                        <img
                          src={item.src}
                          alt={item.title}
                          className="object-cover w-full h-full opacity-60 group-hover:opacity-100"
                        />
                      )}
                    </div>
                  )}
                ...
                {/* Title overlay */}
                <div className="absolute inset-x-0 bottom-0 p-5 translate-y-2 group-hover:translate-y-0 transition-transform duration-300 bg-gradient-to-t from-black via-black/80 to-transparent">
