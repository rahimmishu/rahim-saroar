...
      navigate('/'); // লগআউটের পর হোমে নিয়ে যাবে
    } catch (error) {
      console.error("Logout failed", error);
    }
  };

  // 🔥 ৩. প্রোফাইলে যাওয়ার ফাংশন
  const goToProfile = () => {
    navigate('/profile');
    setIsProfileMenuOpen(false);
    setIsOpen(false);
...
              <button onClick={() => { onOpenTools(); setIsOpen(false); }} className="flex flex-col items-center justify-center p-4 transition-all border border-purple-100 bg-purple-50 dark:bg-zinc-900 dark:border-zinc-800 rounded-2xl active:scale-95">
