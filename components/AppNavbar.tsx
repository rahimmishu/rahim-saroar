  ...
  return (
    <>
      <nav className={`fixed w-full z-[50] transition-all duration-500 ease-in-out ${
        ...
        <div className="container px-4 mx-auto md:px-8">
          <div className="flex items-center justify-between">
            ...
            {/* LOGO */}
            <a href="#home" className="group flex items-center gap-2 relative z-[60]">
               <div className="relative">
                  <Sparkles className="text-purple-500 animate-pulse" size={24} />
                  <div className="absolute inset-0 duration-1000 rounded-full opacity-50 bg-purple-500/40 blur-lg animate-ping"></div>
               </div>
               <span className="text-2xl font-bold text-transparent font-signature bg-gradient-to-r from-slate-800 via-purple-600 to-blue-600 dark:from-white dark:via-purple-400 dark:to-blue-400 bg-clip-text">
                  Rahim Saroar Mishu
               </span>
            </a>
            ...
            <div className="items-center hidden space-x-8 lg:flex">
              <div className="flex items-center gap-4">
                 <button onClick={onOpenGallery} className="flex items-center gap-2 px-4 py-2 font-bold text-purple-600 transition-transform duration-300 bg-purple-100 rounded-full shadow-sm dark:bg-purple-900/30 dark:text-purple-400 hover:scale-105 hover:shadow-purple-500/20">
                 ...
                 <button onClick={onOpenTools} className="flex items-center gap-1 font-medium transition-colors text-slate-600 dark:text-slate-300 hover:text-purple-600 dark:hover:text-purple-400">
                    <Wrench size={18} /> Tools
                 </button>
              </div>
              
              <div className="flex items-center pl-4 space-x-4 border-l border-slate-200 dark:border-slate-800">
                 <button onClick={toggleTheme} className="p-2 transition-all rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:rotate-12">
            ...
            <div className="lg:hidden flex items-center gap-3 z-[60]">
               <button onClick={toggleTheme} className="p-2 transition-transform rounded-full bg-slate-100/50 dark:bg-slate-800/50 backdrop-blur-md text-slate-600 dark:text-slate-300 active:scale-90">
                  {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
               </button>
               <button onClick={() => setIsOpen(!isOpen)} className="p-2 transition-transform text-slate-800 dark:text-white active:scale-90">
      ...
      >
          ...
          {/* Navigation Links List (Staggered Animation) */}
          <div className="relative z-10 flex flex-col w-full max-w-md gap-3 mx-auto">
            {navLinks.map((link, idx) => (
              <a 
                ...
                `}
                style={{ transitionDelay: `${idx * 70}ms` }} // 🔥 STAGGERED DELAY
              >
          ...
          {/* 🔥 WIDGET GRID WITH DELAYED ENTRY 🔥 */}
          <div className={`grid grid-cols-2 gap-4 mt-6 w-full max-w-md mx-auto relative z-10 transition-all duration-700 delay-500 ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
             ...
             <button onClick={() => { onOpenGallery(); setIsOpen(false); }} className="relative overflow-hidden flex flex-col items-start justify-between h-32 p-5 bg-gradient-to-br from-purple-50 to-white dark:from-slate-800 dark:to-slate-900 rounded-[2rem] border border-purple-100 dark:border-slate-700 active:scale-95 transition-all duration-300 hover:shadow-xl hover:shadow-purple-500/10 group">
                <div className="absolute top-0 right-0 w-20 h-20 -mt-10 -mr-10 transition-transform rounded-full bg-purple-500/10 blur-2xl group-hover:scale-150"></div>
                ...
                <div>
                    <span className="block text-lg font-bold text-slate-800 dark:text-slate-200">Gallery</span>
                    <span className="text-xs font-medium text-slate-500">View Photos</span>
                </div>
             </button>
             ...
             <button onClick={() => { onOpenTools(); setIsOpen(false); }} className="relative overflow-hidden flex flex-col items-start justify-between h-32 p-5 bg-gradient-to-br from-blue-50 to-white dark:from-slate-800 dark:to-slate-900 rounded-[2rem] border border-blue-100 dark:border-slate-700 active:scale-95 transition-all duration-300 hover:shadow-xl hover:shadow-blue-500/10 group">
                <div className="absolute top-0 right-0 w-20 h-20 -mt-10 -mr-10 transition-transform rounded-full bg-blue-500/10 blur-2xl group-hover:scale-150"></div>
                ...
                <div>
                    <span className="block text-lg font-bold text-slate-800 dark:text-slate-200">Tools</span>
                    <span className="text-xs font-medium text-slate-500">My Stack</span>
                </div>
             </button>

             {/* Secret Vault Widget (Full Width & Glowing) */}
             <button onClick={triggerSecretVault} className="col-span-2 relative overflow-hidden flex items-center justify-between p-6 bg-slate-900 dark:bg-black rounded-[2rem] active:scale-95 transition-all duration-300 shadow-2xl shadow-slate-400/20 dark:shadow-black/50 group border border-slate-700/50">
                ...
                <div className="relative z-10 flex items-center gap-4">
                  <div className="p-3 text-pink-500 transition-colors bg-slate-800 rounded-2xl group-hover:text-white group-hover:bg-pink-500"><Lock size={22} /></div>
                  <div className="flex flex-col items-start text-left">
                    <span className="text-lg font-bold text-white">Secret Vault</span>
                    <span className="text-xs transition-colors text-slate-400 group-hover:text-pink-200">Tap to unlock hidden features</span>
                  </div>
                </div>
